...
/**
 * @Process(name="RoutingQueueTwoConsumer")
 */
class RoutingQueueTwoConsumer extends AbstractProcess
{
    public const EXCHANGE_NAME = 'test_routing_exchange';

    public const QUEUE_NAME = 'test_routing_queue_2';
    ...
    public function handle(): void
    {
        ...
        $logger->info('routing_queuec2 启动');
        ...
        $channel->basic_consume(self::QUEUE_NAME, '', false, false, false, false, static function (AMQPMessage $message) use ($logger) {
            sleep(1);
            $logger->info('routing_queuec2 消费:'.$message->body.'/'.$message->delivery_info['delivery_tag']);
        ...
        $logger->info('routing_queuec2 关闭');
