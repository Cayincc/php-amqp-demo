...
/**
 * @Process(name="PublishSubscribeQueueThreeConsumer")
 */
class PublishSubscribeQueueThreeConsumer extends AbstractProcess
{
    ...
    public function handle(): void
    {
        ...
        $logger->info('pub_sub_queuec_temporary  启动');
        ...
        //绑定队列到交换机
        $channel->queue_bind($queue_name, self::EXCHANGE_NAME, '', false, [], null);
        //每个消费者发送确认消息之前，消息队列不发送下一个消息到消费者，一次只处理一个消息
        //限制发送同一个消费者不得超过一条消息
        $channel->basic_qos(null, self::PREFETCH_COUNT, null);

        $channel->basic_consume($queue_name, '', false, false, false, false, static function (AMQPMessage $message) use ($logger) {
            sleep(1);
            $logger->info('pub_sub_queuec_temporary 消费:'.$message->body.'/'.$message->delivery_info['delivery_tag']);
        ...
        $logger->info('pub_sub_queuec_temporary 关闭');
