...
/**
 * @Process(name="TopicQueueTwoConsumer")
 */
class TopicQueueTwoConsumer extends AbstractProcess
{
    public const EXCHANGE_NAME = 'test_topic_exchange';

    public const QUEUE_NAME = 'test_topic_queue_2';
    ...
    public function handle(): void
    {
        ...
        $channel->basic_consume(self::QUEUE_NAME, '', false, false, false, false, static function (AMQPMessage $message) use ($logger) {
            sleep(1);
            $logger->info('topicqueuec2 消费:'.$message->body.'/'.$message->delivery_info['delivery_tag']);
